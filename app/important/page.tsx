  ...
  const departmentsWithBranches = useMemo(() => {
    ...
    DEPARTMENTS.forEach((dept) => {
      ...
      dept.branches.forEach((branch) => {
        // Count total important content for this branch
        ...
        <div className="container mx-auto px-4 py-12">
          ...
          <div className="mb-8">
            ...
            <p className="text-muted-foreground text-lg">
              Select a department and branch to view important questions by semester
            </p>
          </div>
                ...
                <div key={departmentSlug} className="space-y-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {branches.map((branch) => (
                      <Link
                        key={branch.id}
                        href={`/important/${departmentSlug}/${branch.slug}`}
                      >
                                ...
                                {branch.contentCount > 0 ? (
                                  <>
                                    ...
                                    <span className="text-foreground font-medium">
                                      {branch.contentCount} {branch.contentCount === 1 ? "item" : "items"}
                                    </span>
                                  </>
                                ) : (
                                  <>

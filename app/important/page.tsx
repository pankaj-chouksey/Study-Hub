...
export default function ImportantPage() {
  ...
  // Filter content by type "important"
  const importantContent = useMemo(() => {
    return allContent.filter((item: Content) => item.type === "important")
  }, [allContent])

  // Group branches by department and get important content for each branch
  const branchesWithContent = useMemo(() => {
    ...
    DEPARTMENTS.forEach((dept) => {
      dept.branches.forEach((branch) => {
        // Filter important content for this specific branch
        const branchContent = importantContent.filter(
          (item: Content) =>
        ...
        <div className="container mx-auto px-4 py-12">
          ...
          <div className="mb-8">
            ...
            <p className="text-muted-foreground text-lg">
              Browse important questions and study materials organized by branch
            </p>
          </div>
              ...
              {branchesWithContent.map(({ department, departmentSlug, branch, content }) => (
                <div key={`${departmentSlug}-${branch.slug}`} className="space-y-4">
                  ...
                  <div className="flex items-center gap-4">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground mt-1">
                        {department} • {content.length} {content.length === 1 ? "item" : "items"}
                      </p>
                    </div>
                  </div>
                  ...
                  {content.length > 0 ? (
                    <ContentList
                      content={content}
                      emptyMessage={`No important content available for ${branch.fullName || branch.name}.`}
                    />
                  ) : (
                    <Card className="border border-[#2E2E2E] dark:border-[#EEEEEE] bg-[#2E2E2E] dark:bg-[#3A3A3A]">
                      <CardContent className="p-8 text-center">
                        <p className="text-[#EEEEEE] dark:text-[#EEEEEE] text-base">
                          No important content available for {branch.fullName || branch.name} yet.
                        </p>
                      </CardContent>
                    </Card>
                  )}

...
import { FileQuestion, Loader2, ChevronRight } from "lucide-react"

export default function PYQsBranchPage() {
  ...
  // Filter PYQ content (type === "pyq")
  const pyqContent = useMemo(() => {
    return allContent.filter((item: Content) => item.type === "pyq")
  }, [allContent])
  ...
  // Get semesters with PYQ content counts
  const semestersWithContent = useMemo(() => {
    return years.map((year) => {
      // Count PYQ content for this specific semester
      const semesterContent = pyqContent.filter(
        (item: Content) =>
  ...
  }, [years, pyqContent, dept?.name, branchData?.name])
        ...
        <div className="container mx-auto px-4 py-12">
          ...
          <div className="mb-8">
            <div className="flex items-center gap-2 text-sm text-muted-foreground mb-4">
              ...
              <Link href="/pyqs" className="hover:text-foreground transition-colors capitalize">
            ...
            <div className="flex items-center gap-3 mb-4">
              <FileQuestion className="w-8 h-8 text-[#2E2E2E] dark:text-[#EEEEEE]" />
              <h1 className="text-3xl sm:text-4xl font-light text-[#2E2E2E] dark:text-[#EEEEEE]">
                {branchData?.fullName || branchData?.name || branch.replace("-", " ")}
              </h1>
            </div>
            <p className="text-muted-foreground text-lg">
              Select a semester to view PYQ content
            </p>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {semestersWithContent.map((semester) => (
                <Link
                  key={semester.level}
                  href={`/departments/${department}/${branch}/${semester.level}?type=pyq`}
                >
                          ...
                          {semester.contentCount > 0 ? (
                            <>
                              ...
                              <span className="text-foreground font-medium">
                                {semester.contentCount} {semester.contentCount === 1 ? "PYQ" : "PYQs"}
                              </span>
                            </>
                          ) : (
                            <>
                              <FileQuestion className="h-4 w-4 text-muted-foreground" />
                              <span className="text-muted-foreground">No PYQs yet</span>
                            </>
                          )}
                        </div>
                      </div>
